    ...
    def create_database_if_not_exists(self) -> None:
        ...
        try:
            ...
            admin_engine = create_engine(
                f"postgresql+psycopg2://{self.database.user}:{self.database.password}@{self.database.host}:{self.database.port}/postgres"
            )
            with admin_engine.connect() as conn:
                ...
                logging.info(f"Base de datos '{self.database.name}' creada en PostgreSQL.")
            ...
            if pgcode == '42P04':
                ...
                logging.info(f"La base de datos '{self.database.name}' ya existe en PostgreSQL.")
            else:
