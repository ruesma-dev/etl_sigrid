            ...
            for table_key in tables_to_transfer:
                try:
                    ...
                    if not extracted_data or source_table not in extracted_data:
                        logging.warning(f"La tabla de origen '{source_table}' está vacía o no se pudo extraer.")
                        continue  # Pasar a la siguiente tabla

                    df = extracted_data[source_table]

                    # Aplicar transformaciones, pasando extract_use_case
                    transformed_df = Transformer.transform(table_key, df, self.extract_use_case)
